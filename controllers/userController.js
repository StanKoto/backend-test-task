...
const users_get = async (req, res, next) => {
  try {
    ...
    if (!Number.isInteger(Number(count))) throw new Error('Wrong count format')
    if (count < 1 || count > 100) throw new Error('Wrong count value')
    if (typeof offset === 'undefined') {
      if (typeof page === 'undefined') offset = 0, page = 1
      if (!Number.isInteger(Number(page))) throw new Error('Wrong page format')
      if (page < 1) throw new Error('Wrong page value')
      offset = (page - 1) * count;
    } else {
      if (!Number.isInteger(Number(offset))) throw new Error('Wrong offset format')
      if (offset < 0) throw new Error('Wrong offset value')
      page = Math.ceil(offset / count) + 1;
    }

    const total_users = await User.count();
    if (total_users <= offset) throw new Error('Offset too large')
    ...
    const users = await User.findAll({ 
      ...
      // include: { association: 'position', attributes: [ 'name' ] },
      offset,
      limit: count
    });
...
const users_post = async (req, res, next) => {
  try {
    ...
    if (existingUser) throw new Error('Token already used')
    req.body.token_id = decodedToken.id;
    
    const photo = req.file;
    if (!photo) throw new Error('User photo missing')
    const photoParams = await sharp(photo.buffer).metadata();
    if (photoParams.width < 70 || photoParams.height < 70) throw new Error('Wrong image size')
...
const user_get = async (req, res, next) => {
  try {
    if (!Number.isInteger(Number(req.params.id))) throw new Error('Wrong user id format')
